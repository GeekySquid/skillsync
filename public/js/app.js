...
function displayResults(result) {
  ...
  // Calculate counts
  // FIX: Client-side deduplication to ensure UI is clean even if server returns duplicates
  if (result.matchedSkills && result.missingSkills) {
    const matchedSet = new Set(result.matchedSkills.map(s => s.toLowerCase().trim()));

    // Filter missing skills
    result.missingSkills = result.missingSkills.filter(
      skill => !matchedSet.has(skill.toLowerCase().trim())
    );

    // Filter priorities
    if (result.skillPriority) {
      ['critical', 'important', 'optional'].forEach(priority => {
        if (Array.isArray(result.skillPriority[priority])) {
          result.skillPriority[priority] = result.skillPriority[priority].filter(
            skill => !matchedSet.has(skill.toLowerCase().trim())
          );
        }
      });
  ...
  const coursesCount = result.skillCourses?.reduce((total, sc) => total + (sc.courses?.length || 0), 0) || 0;

  // Update stat cards
  const matchedCountEl = document.getElementById("matchedCount");
  const missingCountEl = document.getElementById("missingCount");
  const coursesCountEl = document.getElementById("coursesCount");
  const matchedBadgeEl = document.getElementById("matchedBadge");
  const missingBadgeEl = document.getElementById("missingBadge");
  ...
  if (matchedBadgeEl) matchedBadgeEl.textContent = matchedCount;
  if (missingBadgeEl) missingBadgeEl.textContent = missingCount;
  ...
  // Display course recommendations for skill gaps
  const coursesSection = document.getElementById("coursesSection");
  const coursesList = document.getElementById("coursesList");

  if (result.skillCourses && result.skillCourses.length > 0 && coursesSection && coursesList) {
    ...
    coursesList.innerHTML = result.skillCourses
      .map((skillCourse) => {
        const coursesHTML = skillCourse.courses
          .map((course) => {
            const icon = platformIcons[course.platform] || "📖";
            const typeClass = course.type === "free" ? "free" : "paid";
            const typeLabel = course.type === "free" ? "FREE" : "PAID";
        ...
        return `
          <div class="skill-courses">
            <h4 class="skill-courses-title">📌 ${skillCourse.skill}</h4>
